    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="ml-2 text-[10px] px-2 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 font-medium">PRO</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link href="/admin/login" className="hidden sm:inline-flex px-3 py-2 rounded-xl border border-slate-200 text-sm">Owner Login</Link>
          </div>
        </div>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="text-slate-600 mt-4 max-w-xl">Upload bank + Tally/Excel, auto-match in seconds, flag anomalies, chase clients on WhatsApp, export audit-ready. Built for Chartered Accountants.</p>
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Link href="/admin/login" className="px-6 py-3 rounded-xl border border-slate-200 hover:bg-slate-50 font-medium">Owner Access</Link>
            </div>
            <p className="text-xs text-slate-400 mt-3">Public landing — no stats here. Dashboard is private (Activities only for you).</p>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-16 grid md:grid-cols-3 gap-6">
        {[
          ...
          { title: 'WhatsApp chase', desc: 'One-tap client nudges from Insights with pre-filled text.' },
        ].map(c => (
      ...
      <footer className="border-t border-slate-200 py-8 text-center text-xs text-slate-400">CA-Flow AI • Secure • India-hosted • <Link href="/dashboard" className="underline">Dashboard</Link> is private</footer>
    </div>
  );
}
